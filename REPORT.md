# Отчет: Track Forensics по профессии «Специалист по информационной безопасности». 
...
Исследование представленных материалов позволило восстановить следующую картинку инцидента:
...
* С помощью вредоносной программы злоумышленники могли осуществлять сбор информации о системе. Кроме установки дополнительных программ и обновлений было запущено скачивание по-видимому вредоносных программ.  
...
* В ходе атаки был создан новый пользователь - Support с повышенными привилегиями, под которым выполнялись многие действия.   
* Для дальнейшего закрепления пользователь Support манипулирует с планировщиком задач - Task Scheduler, в результате чего запускается вредоносный скрипт Update.bat, ограничиваются возможности сканера Windows Defender.
* В ходе инцидента злоумышленниками исследуются архитектура сети, порты TCP/UDP, сервисы на них, собирается информация через сетевые протокола, злоумышленниками выявлены пароли легитимного пользователя рабочей станции.
...
*Рис.1а. История загрузок ресурсов через браузер Internet Explorer*  
...
После его экспорта на хостовую машину с Windows 11, при попытке работы с ним, Windows Defender сообщает об угрозе (Рис. 3).  
...
Некоторые вендоры описывают его как trojan. На рис. 5 представлены тактики MitreAtt&ck, в которых он применяется злоумышленниками.  
...
Таким образом, выявлено, что загруженный файл является вредоносом. Кроме того, можно выдвинуть гипотезу о том, что на рабочей станции пользователя Wilford был или отключен Windows Defender или настроено разрешение на запуск вредоносных программ.  
...
Исследование Timeliner, извлеченного из RAM dump, позволило выявить, что загруженный malware был запущен 2019-03-09 в 14:31:04 UTC+0000 (Рис. 6) или в 17:31:04 MSC. 
...
При анализе файла *$MFT* на предмет дальнейшей злонамеренной активности, выясняется, что именно он был запущен от имени пользователя Wilford первым в чреде дальнейшей подозрительной деятельности по тайм-лайну (рис. 7) .  
...
Можно предположить, что в качестве дополнительных технологий Mitre Att&ck использовались технологии Discovery (Исследование) Network Service Discovery (T1046) и Password Policy Discovery (T1201). 
...
Как упоминалось выше в папке *C:\Windows\Temp* тоже следы деятельности злоумышленников (Рис. 11). Это приложения *SDelete*, инструмент командной строки, который позволяет безопасно удалять файлы и папки.  
  
*SDelete* перезаписывает удалённые файлы на диске, чтобы сделать их неустранимыми даже с помощью технологий восстановления. Тем самым можно предположить, что в данном случае применялась технология T1070	Indicator Removal. Единственное, подозрительно выглядит время модификации приложения.  
  
Кроме того, в папке *C:\Windows\Temp* можно обнаружить файлы *1*, *111.bat*, *2.bat*, *3.bat*, архивы утилит *iepv.zip*, предназначенная для выгрузки паролей из браузера Internet Explorer, *netscan_portable.zip* – предназначенная для TCP-сканирования сети, *RDPWInst-v1.6.2.msi* программа для организации удаленного подключения (Рис. 11).
...
Общий итог: запуск загруженное приложения запустил ряд событий, включая скачивание вредоносного и легитимного программного обеспечения, удаление файлов, очистку журнала Windows Defender.
...
Из анализа журнала Remote Connection Manager, было выявлено, что осуществлено успешное удаленное подключение от имени локального пользователя Wilfred c IP-адреса 192.168.1.71 (Рис. 18).  
...
По информации из файла реестра *SOFTWARE.dat* на рабочей станции была установленные программа RDP Wrapper Library:
...
09-02-2019 пользователь wifred удаленно залогинился. Но при попытке войти в учётную запись, используя учётные данные (имя пользователя и пароль), отличные от тех, которые использует текущий пользователь, сеанс был завершен (Рис. 18b).  
...
Стоит отметить, что запуск *ipve.exe* привел к появлению файла *password.txt* на рабочем столе пользователя (Рис. 19).
...
На рис. 19 представлена последовательность запуска сканеров.  
...
Запуск утилит сканирования сетевой инфраструктуры и выявления паролей пользовтеля состоялись с 6:31:24 до 6:41:06 (2019-03-10). Пожтверждается это соответствующей записью в *NTUSER.dat*:  
*OpenSavePidlMRU*  
...
*Рис. 22. Hash Dump извлеченный из dump RAM*  
  
Анализируя приведенные хэши, можно сделать несколько выводов относительно паролей и настроек безопасности в системе. Обе учетные записи ("Wilfred" и "Support") имеют одинаковые LM-хэши: aad3b435b51404eeaad3b435b51404ee. Это стандартный LM-хэш пустой строки или пароля длиной менее 14 символов. Причина проста: LM-хэш обрабатывает пароль таким образом, что, если пароль короче 14 символов, он дополняется нулями до длины 14 символов, а затем разделяется на две части по 7 символов. Эти части обрабатываются отдельно. 

Поскольку оба пользователя имеют короткие пароли, первые 7 символов обеих частей будут одинаковыми (или нулевыми). Таким образом, пароль у Wilfrod очень простой и может быть легко подобран по хэшу. Даже онлайн cracker позволил подобрать пароли администратора и пользователя.    
...
Исследование различных материалов из представленных копий жесткого диска и оперативной памяти о запусках исполняемых файлов и скриптов, изменений файлов реестра, загрузок показывают несоответствие временных меток событий, проихсодящих в ходе атаки. Где-то нарушается последовательность действий и временной комплайнс. Это позволяет сделать вывод, о том, что атакующими были изменений временные метки для сокрытия своих следов. Рассмотрим некоторые факты, свидетельствующие в пользу этой гипотезы.  

**Обращает на себя внимание тот факт, что все события имели место в субботу-воскресение, 9-10 марта 2019 года. До момента 17:17:41 2019-03-09 в журнале Security отсутствует информация о логине пользователя Wilfred, однако действия от него исполняются. В 17:17:41 2019-03-09 появляется запись о том, что меняется аккаунт пользователя (Рис. 22с).**

<img width="2400" height="1420" alt="image" src="https://github.com/user-attachments/assets/2a8c538f-1c0d-47b0-bfe7-20073e752ab2" />  
  
*Рис. 22с. Первый отмеченный в логах логин пользователя Wilford*  
  
Кроме того, в журнале Security вообще отсутствуют записи о событиях начиная с 2018-03-2018 по 17:17:07 2019-03-09, что выглядит очень подозрительно и наводит на мысль об удалении этой информации (Рис. 22d).  
  
<img width="1723" height="524" alt="image" src="https://github.com/user-attachments/assets/7f740f0d-a635-457d-b109-eb87060681d7" />  
  
*Рис. 22d. Отсутствие записей в журнале Security*  
...
На себя обращает внимание разброс времени: запись в NTUSER.dat имеет временную метку **2019-03-09 в 16:57:35Z** при том, что запуск Refund_form состоялся почти за 2 часа 28 минут до этого, в **14:31:04 2019-03-09**. 
  
Запуск Updater.bat осуществился в **12:30:35 2019-03-10**, a загруженны malware-файлы (1, 2.bat, 111.bat, iepv.zip, netscan_portable.zip) и RDPWInst-v1.6.2.msi были в период с **15:52:03 09.03.2019** до **7:07:52 10.03.2019**, то есть до запуска скрипта Updater.bat, в ходе выполнения которого должна была осуществить их загрузка. Например, запись в UserClass о работе iepv.zip была внесена в 6:40, нерабочее время:  
...
Кроме того, в журнале Security обнаружена запись об изменении системного времени (Рис. 22e).
<img width="1471" height="766" alt="image" src="https://github.com/user-attachments/assets/e274fc28-06bb-4639-a1b6-06a033c1d21b" />
*Рис. 22e. Запись об изменении системного времени*
...
Таким образом, можно сделать вывод, что в ходе атаки была использована тактика связанная с изменением временных меток - Timestamping. 
...
Процессы, которые представлены на рис. 26 подтверждают сделанные выводы из анализа dump оперативной памяти по поводу инъекций процессов. Кроме того, выявлен процесс taskhost.exe, в котором реализуется взаимодействие с ресурсом sendspace. 
...
Утилита volatility yarascan c правилом yara позволяла извлечь информацию о многочисленных обращениях к ресурсу процесса *taskhost.exe* (несколько десятков раз за 09.03.2019), который, хотя и является легитимным для операционной системы windows, но может быть использован как маскировка для многих malware-процессов.
...
Для наглядности инцидент далее представлен *маппинг действий атакующих по матрице MITRE ATT&CK* — в виде таблицы, отражающей соответствие тактики и техник, которые использовали атакующие в рамках исследуемого инцидента, временем и пользователем от имени которого осуществлялись действия.  
  
При построение таблицы действия атакующих были выстроены или в хронологическом порядка или исходя из логики. Но, следуюет обратить внимание, что в некоторых случаях логика и хронология не совпадают. Это связана с тем, что было изложено в разделе **2.4.1 Изменение временных меток**. Некоторые этапы *матрицs MITRE ATT&CK* так же переставлены с учетом логики событий и временных показателей.

|Этап|Техника | Реализация техники | Описание |Дата и время инцидента (MSC)| Пользователь|
|---|---|---|----|----|----|
|**Initial Access**|Phishing: Spearphishing Attachment, T1566.001|Злоумышленники могут отправлять электронные письма с вредоносным вложением в попытке получить доступ к системам жертвы. При открытии вложения вредоносная программа использует уязвимость или запускается непосредственно в системепользователя|Легитимный пользователь загрузил с почтового сервера Яндекс архив *Refund_form.zip*|2019-03-09, 17:25:50|Wilfred|
|**User Execution**|Malicious File, T1204.002|Злоумышленники ожидают, что пользователь откроет вредоносный файл, чтобы получить доступ к его выполнению|Легитимный  пользователь разархивировал загруженный файл *Refund_form.zip* и запустил исполняемый файл. В результате чего был выполнен powershell скрипт, закодированный по стандарту base64 и на целевую машину были загружены ряд исполняемых файлов и bat-файлов |2019-03-09, 17:31:04|Wilfred|
|**Defence Ivasion**|Clear Windows Event Logs, T1070.001|Злоумышленники отключают ведение журнала событий Windows и/или удалить события из журналов|Удаление истории WindowsDefender из журнала Microsoft-Windows-Windows Defender%4Operational.evtx|2019-03-09, 17:25:50|Wilfred|
...
|**Defence Evasion**|Exploitation for Defense Evasion, T1211|Злоумышленники могут использовать уязвимость системы или приложения для обхода средств защиты|Запуск Updater.bat, ограничивающего Windows Defender, осуществлена после логина легального пользователя|2019-03-10, 05:56:46 |Wilfred|
|**Defense Evasion**|Timestomp T1070.006|Злоумышленники могут изменять временные атрибуты файлов, чтобы скрыть новые файлы или изменения в существующих файлах|Под легальным юзером было изменено системное время с 20:35:31 2019-03-09 на 08:05:30 2019-03-10|2019-03-09, 20:35:31|Wilfred|
|**Defence Ivasion**|File Deletion, T1070.004|Злоумышленники могут удалять файлы, оставленные в результате их действий по вторжению|С рабочей станции удалено более 500 ресурсов|2019-03-09, 17:18 - 2019-03-10, 14:04|Wilfred, Support|
|**Discovery**|Network Service Discovery, T1046|Злоумышленники могут попытаться получить список служб, запущенных на удаленных хостах и устройствах инфраструктуры локальной сети|Запущена программа netscan_portable, инструмент для сканирования и анализа сети на подключённые устройства и сервисы |2019-03-10, 06:35:43-51|Wilfred|
|**Discovery**|Password Policy Discovery, T1201|Злоумышленники могут попытаться получить информацию о паролях легальных пользователей |Запущена программа iepv, которая позволяет выявить пароли и иные данных легальных пользователей. На рабочем стале пользователя Wilfred появляется файл Password.txt|2019-03-10, 06:41:06|Wilfred|
|**Lateral Movement**|Windows Remote Management, T1021.006|Злоумышленники могут использовать действительные учетные записи для взаимодействия с удаленными системами с помощью Windows Remote Management и выполнять действия, которые используются для входа в систему|Из под легального пользователя осуществляется удаленный вход через IP-адрес *192.168.1.71* - который соответствует информации *COPE-репорт*|2019-03-09, 19:35:27 и 19:55:12|Wilfred|
|**Privilege Escalation**|Valid Accounts: Local Accounts, T1098.003| Получение доступа к аккаунту легитимного пользователя Wilfred|Путем запуска программы iepv были получены данные легального пользователя с административными правами - об этом может свидетельсовать появление на рабочем столе пользователя файла Password.txt, который был открыт 09.03.2019 в 15:35:28|2019-03-10, 06:41:06|Wilfred|
|**Persistence**|Create Account: Local Account, T1136.001|Злоумышленники могут создать локальную учетную запись для обеспечения доступа к системам-жертвам|Из под легального пользователя создаетcя пользователь Support|2019-03-10, 06:45:48|Wilfred|
|**Privilege Escalation**|Account Manipulation, T1098.003|Злоумышленники могут повышать привилегии пользователям, чьи данные известны|Повышение привилегий пользователя Support и включение в группу безопасности |2019-03-09, 17:17:17|Wilfred|
|**Privilege Escalation**|Process Injection, T1055||Инъекции в различные процессы обнаружены при исследовании dump RAM|2019-03-09 - 2019-03-10|Wilfred, Support|
|**Exfiltration**|Exfiltration Over Web Service: Exfiltration to Cloud Storage, T1567.002|С её помощью злоумышленник пытается скрытно выгрузить данные из целевой системы за пределы периметра организации через облачные сервисы хранения|Как выяснилось, для организации утечки информации использовался ресурс <span></span>sendspace.com – ресурс для обмена файлами, в том числе и большими|2019-03-10, 6:58:46, 7:06:55, 7:07:44, 12:29:54|Wilfred|

Стоит отметить еще один признак несоответствия временных меток. В частности события, связанные с созданием пользователя Support не соответствуют дальнейшим действиям по повышению привилегий этого пользователя. В частности согласоно логам он был создан 2019-03-10 в 06:45:48, а действия по повышению его привилегий состоялись 2019-03-09, в 17:17:17. При этом, настройка Schеdule состоялась 2019-03-10 в 05:56:46.
